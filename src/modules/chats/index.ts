		...
		group
			...
			.get(
				...
				async (context) => {
					const user = (context as typeof context & { user?: { id: bigint } }).user;
					if (!user) throw new Error('Unauthorized');
					return await ChatService.listHistory(user.id);
				},
				{
					detail: { tags: ['Chats'], summary: 'List chat history' },
				},
			)
			.post(
				'/:id/pause',
				async (context) => {
					const user = (context as typeof context & { user?: { id: bigint } }).user;
					if (!user) throw new Error('Unauthorized');
					return await ChatService.pause(user.id, context.params.id);
				},
				{
					params: t.Object({ id: ID_SCHEMA }),
					detail: { tags: ['Chats'], summary: 'Pause a chat' },
				},
			)
			.post(
				'/:id/end',
				...
				{
					params: t.Object({ id: ID_SCHEMA }),
					detail: { tags: ['Chats'], summary: 'End a chat' },
				},
			)
			.get(
				'/search',
