...
export class Dragonfly {
	...
	/** Atomically read and delete — avoids races on one-time tokens. */
	public async getdel<T>(key: string): Promise<T | null> {
		const data = await this.pub.getdel(key);
		if (!data) {
			return null;
		}
		...
		try {
			...
			console.error('Failed to parse JSON from Redis (getdel)', { key, data, error });
			return null;
		}
	}
